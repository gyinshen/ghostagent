...
from repository.report.agent.llm_utils import create_chat_completion
import os
...
def split_text(text: str, max_length: int = 16384) -> Generator[str, None, None]:
...
def summarize_text(
    ...
    scroll_ratio = 1 / len(chunks) if driver else 0

    for i, chunk in enumerate(chunks):
        if driver and url.endswith('.pdf') is False:
...
def scroll_to_percentage(driver: WebDriver, ratio: float) -> None:
    ...
    target_height = total_height * ratio
    driver.execute_script(f"window.scroll(0, {target_height});")
